...
hostname: apollo.io
description: Overview Apollo offers a highly accurate database of hundreds of millions of people, enabling you to find and connect with your ideal customers across the world. The People API Search enables you to find the prospects most likely to benefit from your product or service through targeted search filter…
...
CreditsUsing this endpoint doesn't consume your account’s credits.
...
|---|---|---|
| ["sales director", "director sales", "director, sales"] | Your search filters need to be added as an array of strings. The |
| ["California, US", "Oregon, US", "Washington, US"] | Your search filters need to be added as an array of strings. The |
| 5 | Your value should be an integer between 1 and 100. This parameter can be used to specify how many results to return per page. |
...
response returns JSON data as shown below.

To get complete person profiles, use the `id`

values from this response and include them in the `details`

array when calling the `people/bulk_match`

endpoint. For more details and usage examples, refer to the Enrich People Data tutorial.
